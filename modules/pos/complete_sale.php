...
try {
    $txn_stmt = $conn->prepare("INSERT INTO transactions (user_id, payment_type, total_amount) VALUES (?, ?, ?)");
    $txn_stmt->bind_param("isd", $user_id, $payment_type, $total_amount);
    $txn_stmt->execute();
    ...
    echo json_encode([
        'success' => true,
        'receipt_url' => "receipt.php?transaction_id={$transaction_id}"
    ]);
} catch (Exception $e) {
    $conn->rollback();

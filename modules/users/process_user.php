...
if ($action === 'create') {
    ...
    if ($password !== $confirm_password) {
        header("Location: user_form.php?error=Passwords do not match");
        exit();
    }
    
    if (strlen($password) < 6) {
        header("Location: user_form.php?error=Password must be at least 6 characters");
        exit();
    }
    ...
    if ($check_stmt->get_result()->num_rows > 0) {
        header("Location: user_form.php?error=Email already exists");
        exit();
    }
    ...
    if ($stmt->execute()) {
        ...
        header("Location: user_form.php?error=Failed to create user");
    }
    exit();
}
...
elseif ($action === 'update') {
    ...
    if (empty($name) || empty($role)) {
        header("Location: user_form.php?id=$user_id&error=All fields are required");
        exit();
    }
    ...
    if (!empty($password)) {
        if ($password !== $confirm_password) {
            header("Location: user_form.php?id=$user_id&error=Passwords do not match");
            exit();
        }
        
        if (strlen($password) < 6) {
            header("Location: user_form.php?id=$user_id&error=Password must be at least 6 characters");
            exit();
        }
    ...
    if ($stmt->execute()) {
        ...
        header("Location: user_form.php?id=$user_id&error=Failed to update user");
    }
    exit();
}

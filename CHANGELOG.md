...
### Rationale: Enhance Installment Plan Functionality

*   **Chosen Approach:** Implemented a robust installment payment workflow that includes down payments, administrative authorization, and detailed receipt breakdowns.
*   **Reason for Decision:** The previous system lacked the ability to handle down payments, verify admin approval for credit sales, and provide transparent documentation for customers. These additions ensure financial accuracy, security, and better customer service.

### Added
-   **Database Schema:** Added `down_payment` column (DECIMAL 10,2) to `transactions` table in `scripts/tialo_posdb.sql`.
-   **Security:** Added `src/modules/auth/verify_admin_password.php` to verify admin credentials before approving installment sales.
-   **POS UI:** Added "Down Payment" input and dynamic breakdown display to the POS interface.
-   **Receipt:** Enhanced the receipt to show down payment, remaining balance, and a clear installment schedule.

### Changed
-   **POS Logic:** Updated `src/modules/pos/complete_sale.php` and `src/modules/pos/process_checkout.php` to calculate installments based on the balance after down payment.
-   **POS UI:** Made customer details mandatory for installment sales.

### Rationale: Track Shipment Profitability

*   **Chosen Approach:** Added a `cost` column to the `shipments` table and created a comprehensive "Shipment Profitability Report". The report calculates profit by subtracting the shipment's total cost from the revenue generated by selling the items within that shipment. It includes both a summary list and a detailed drill-down view.
*   **Reason for Decision:** The business operates on a "blind box" bidding model where the cost is incurred at the shipment level, but revenue comes from individual item sales. This feature allows the owner to track the return on investment (ROI) for each specific shipment, which was previously impossible without manual calculation.

### Added
-   **Database Schema:** Added `cost` column (DECIMAL 10,2) to `shipments` table in `scripts/tialo_posdb.sql`.
-   **Report:** Created `src/modules/reports/shipment_report.php` to display shipment financials (Cost, Revenue, Profit, ROI).
-   **UI:** Added "Shipment Reports" tab to `src/modules/reports/index.php`.

### Changed
-   **Inventory:** Updated `src/modules/inventory/shipment_form.php` and `process_shipment.php` to include the `cost` field during creation and editing.
...
*   **Chosen Approach:** Implemented visual feedback for user actions, enhanced empty states, and added keyboard shortcut discoverability in the POS system.
